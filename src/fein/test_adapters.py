...
class TestAltLora:
    ...
    def test_only_adapter_layers_are_updated(self, model_cls, config):
        ...
        peft_model = self.get_peft_model(model_cls, config)
        params_before = {
            k: v.clone().detach() for k, v in peft_model.named_parameters()
        }
        self.fit(peft_model, X, y)
        ...
        for key, param_before in params_before.items():
            ...
            is_module_to_save = any(
                key.startswith(m2s + ".new_module") for m2s in config.modules_to_save
            )
    ...
    def test_multiple_adapters_only_active_one_is_updated(self, model_cls, config):
        ...
        params_A_before = {
            k: v.clone().detach()
            for k, v in peft_model._adapter_registry[
                "adapter-A"
        ...
        params_A_after = dict(
            peft_model._adapter_registry["adapter-A"].named_adapter_parameters()
        )
        params_B_after = dict(
            peft_model._adapter_registry["adapter-B"].named_adapter_parameters()
        )
    ...
    def test_multiple_adapters_output(self, model_cls, config):
        ...
        prefix = get_prefix_from_config(config)
        grad_dict = {
            name: param.requires_grad for name, param in peft_model.named_parameters()
        }
        grad_dict_expected = {
            name: (prefix in name) or ("new_module" in name) for name in grad_dict
        }
        ...
        self.fit(peft_model, X, y, epochs=10)
    ...
    def test_mixing_adaptations_parameters_works(self):
        ...
        lora_layers = [
            layer for layer in peft_model.modules()
            if isinstance(layer, LinearLoraLayer)
        ]
    ...
    def test_mixing_adaptations_types_works(self):
        ...
        ia3_layers = [
            layer for layer in peft_model.modules() if isinstance(layer, LinearIA3Layer)
        ]
